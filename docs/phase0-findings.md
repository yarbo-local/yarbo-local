...
**Ultrasonic avoidance.** `ultrasonic_msg.lf_dis` and `rf_dis` read `-3` when nothing echoes and 166 to about 1,800 while the robot closes on something, falling second by second (1548, 1334, 1123, 945, 827, 736) before it reverses; `mt_dis` read `0` or `9999` throughout, so the middle sensor appears unused or disabled. The values fit millimetres. In 150 s of the West Lawn run, with the barrier list empty, these readings form 18 approach episodes at 15 distinct places. This is the avoidance the user saw. `yarbo_local.obstacles.ObstacleTracker` logs each episode at its closest reading, placed from the robot pose and an estimated sensor mounting (0.6 m forward, 0.22 m to the side, 20 degrees outward; not measured), merges repeats within 0.5 m, and keeps barrier clusters alongside, per plan run. Fixture: `mower-pro-ultrasonic-avoidance.jsonl`.
