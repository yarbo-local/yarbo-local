...
- **Stop, effects only.** Stop pressed in the app at 50 percent of the West Lawn: the state read exactly like a pause (planning 1 to 0, `planning_paused` 1, "Goal canceled "), and four seconds later `planning_paused` went back to 0 with planning still 0. That second step is the stop. The robot stayed where it was. The command itself was not seen: for that minute the app published nothing on the local broker, keep-alives included, as with the first pause that day. Fixture: `plan-stopped-from-app-effects-only.jsonl`.
